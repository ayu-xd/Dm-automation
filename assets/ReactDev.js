! function() {
  ...
  new class {
    ...
    async _enterUsername({
      ...
      if (r) try {
        var s = r.findOne("IGDSBox.react").findOne("IGDSBox.react").element.child.child.child.child.sibling;
        if (!s) {
    ...
    _getDatabase(e) {
      var t = this.root.findOne("ReStoreProvider.react");
      return require("ReQL").toArrayAsync(require("ReQL").fromTableAscending(t.element.memoizedProps.db.tables[e]))
    }
    _getDatabaseIterator(e) {
      return this.root.findOne("ReStoreProvider.react").element.memoizedProps.db.tables[e].entries()
    }
